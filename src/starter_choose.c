...
const int GRASS_STARTER_CHOICE [] = {SPECIES_RAYQUAZA, SPECIES_BULBASAUR, SPECIES_CHIKORITA, SPECIES_TREECKO, SPECIES_TURTWIG, SPECIES_SNIVY, SPECIES_CHESPIN, SPECIES_ROWLET, SPECIES_GROOKEY, SPECIES_SPRIGATITO};
const int FIRE_STARTER_CHOICE [] = {SPECIES_GROUDON, SPECIES_CHARMANDER, SPECIES_CYNDAQUIL, SPECIES_TORCHIC, SPECIES_CHIMCHAR, SPECIES_TEPIG, SPECIES_FENNEKIN, SPECIES_LITTEN, SPECIES_SCORBUNNY, SPECIES_FUECOCO, SPECIES_CYNDAQUIL_DELTA};
const int WATER_STARTER_CHOICE [] = {SPECIES_KYOGRE, SPECIES_SQUIRTLE, SPECIES_TOTODILE, SPECIES_MUDKIP, SPECIES_PIPLUP, SPECIES_OSHAWOTT, SPECIES_FROAKIE, SPECIES_POPPLIO, SPECIES_SOBBLE, SPECIES_QUAXLY};
...
u16 GetStarterPokemon(u16 chosenStarterId)
{
    ...
    // int randStarter = RandomUniform(RNG_NONE, 0, 9);

    if (chosenStarterId == 0){
        while (grassChoice == 0){
            grassChoice = RandomUniform(RNG_NONE, 1, 9);
        }
        return GRASS_STARTER_CHOICE[grassChoice];
    }
    else if (chosenStarterId == 1){
        while (fireChoice == 0){
            fireChoice = RandomUniform(RNG_NONE, 1, 10);
        }
        return FIRE_STARTER_CHOICE[fireChoice];
    }
    else if (chosenStarterId == 2){
        while (waterChoice == 0){
            waterChoice = RandomUniform(RNG_NONE, 1, 9);
        }
        return WATER_STARTER_CHOICE[waterChoice];
    }
